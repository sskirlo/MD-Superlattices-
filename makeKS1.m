...
width=83/2; %width of region in angstroms
gap=2.4203; 
...
%calculate len13 so has repeat distance so each boundary is equivalent
...
len11=38/(2)^(1/2); 
len12=44/2*(3/2)^(1/2);
%rem=mod(width/a01,3^(1/2)); 
len13=width/a01; 
%len13=len13; %add in additional row so exactly off by one row in matching
...
len21=34*(3)^(1/2)/2; 
len22=36*(2/3)^(1/2);
rem=mod(width/a02,2^(1/2)); 
%len23=width/a02-rem+0.05; %make so will fit in even distance
len23=width/a02; 
%len23=len23-1/(2)^(1/2); 
